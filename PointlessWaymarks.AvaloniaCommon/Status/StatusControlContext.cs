...
public partial class StatusControlContext
{
    ...
    public static async Task<StatusControlContext> CreateInstance()
    {
        await ThreadSwitcher.ResumeBackgroundAsync();

        var factoryToast = await AppToastContext.CreateInstance();
        return new StatusControlContext(factoryToast);
    }
...
#pragma warning restore 4014
    ...
    public async Task<string> ShowMessage(string title, string? body, List<StatusControlMessageButton>? buttons)
    {
        ...
        Log.ForContext("MessageBoxReturn", toReturn).ForContext(nameof(StatusControlContextId), StatusControlContextId)
...
#pragma warning restore 4014
    ...
    public async Task ToastError(string? toastText)
    {
        await Toast.Show(toastText, ToastType.Error);
        _ = Task.Run(() => Log.Error("Toast Error: {0} - Status Context Id: {1}", toastText, StatusControlContextId));
    }

    public async Task ToastSuccess(string? toastText)
    {
        await Toast.Show(toastText, ToastType.Success);
        _ = Task.Run(() =>
            Log.Information("Toast Success: {0} - Status Context Id: {1}", toastText, StatusControlContextId));
    }

    public async Task ToastWarning(string? toastText)
    {
        await Toast.Show(toastText, ToastType.Warning);
        _ = Task.Run(() => Log.Warning("Toast Warning: {0} - Status Context Id: {1}", toastText, StatusControlContextId));
    }
