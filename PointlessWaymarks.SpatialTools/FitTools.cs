...
public static class FitTools
{
    ...
    public static Feature LineFeatureFromFitRoute(GpxTools.GpsRouteInformation routeInformation)
    {
    ...
    public static Feature LineFeatureFromFitRouteBuffered(GpxTools.GpsRouteInformation routeInformation, double bufferInFeet)
    {
        if (bufferInFeet <= 0) return LineFeatureFromFitRoute(routeInformation);
    ...
    public static Feature LineFeatureFromFitTrack(GpxTools.GpsTrackInformation trackInformation)
    {
        // ReSharper disable once CoVariantArrayConversion
    ...
    public static Feature LineFeatureFromFitTrackBuffered(GpxTools.GpsTrackInformation trackInformation, double bufferInFeet)
    {
    ...
    public static Task<bool> FitFileHasLocationData(FileInfo fitFile, IProgress<string>? progress = null) =>
        FitFileHasLocation(fitFile, progress);

    public static Task<bool> FitFileHasLocations(FileInfo fitFile, IProgress<string>? progress = null) =>
        FitFileHasLocation(fitFile, progress);

    public static Task<bool> FitFileHasTrackOrCourse(FileInfo fitFile, IProgress<string>? progress = null) =>
        FitFileHasLocation(fitFile, progress);

    public static Task<bool> FitFileHasLatLong(FileInfo fitFile, IProgress<string>? progress = null) =>
        FitFileHasLocation(fitFile, progress);

    public static async Task<GpxTools.GpsRouteInformation?> RouteInformationFromFitFile(FileInfo fitFile, IProgress<string>? progress = null)
    {
        if (fitFile is not { Exists: true }) return null;

        progress?.Report($"Reading FIT course file {fitFile.FullName}");
        ...
        return await Task.Run(() =>
        {
            ...
            foreach (var rec in data.Records)
            {
                ...
                pointList.Add(new CoordinateZ(lon, lat, elevation));
            }
            ...
            if (pointList.Count == 0)
            {
                foreach (var cp in data.CoursePoints)
                {
                    ...
                    pointList.Add(new CoordinateZ(lon, lat, 0));
                }
            }

            if (pointList.Count == 0) return null;

            var courseNameBytes = data.Course?.GetName();
            var courseName = courseNameBytes != null
                ? System.Text.Encoding.UTF8.GetString(courseNameBytes).TrimEnd('\0', ' ')
                : string.Empty;

            var name = !string.IsNullOrWhiteSpace(courseName) ? courseName : Path.GetFileNameWithoutExtension(fitFile.Name);
            var sport = data.Course?.GetSport()?.ToString();
            var description = !string.IsNullOrWhiteSpace(sport) && sport != "Generic" && sport != "Invalid" ? $"Course - {sport}" : "Course";

            return new GpxTools.GpsRouteInformation(name, description, pointList);
        }).ConfigureAwait(false);
    }

    public static Task<GpxTools.GpsRouteInformation?> RouteInformationFromFit(FileInfo fitFile, IProgress<string>? progress = null) =>
        RouteInformationFromFitFile(fitFile, progress);

    public static async Task<Feature?> RouteLineFromFitFile(FileInfo fitFile, IProgress<string>? progress = null)
    {
        var routeInfo = await RouteInformationFromFitFile(fitFile, progress).ConfigureAwait(false);
        return routeInfo == null ? null : LineFeatureFromFitRoute(routeInfo);
    }

    public static Task<Feature?> RouteLineFromFit(FileInfo fitFile, IProgress<string>? progress = null) =>
        RouteLineFromFitFile(fitFile, progress);

    public static async Task<GpxTools.FeatureAndBufferedFeature?> RouteLineFromFitFileBuffered(FileInfo fitFile, double bufferInFeet, IProgress<string>? progress = null)
    {
        var routeInfo = await RouteInformationFromFitFile(fitFile, progress).ConfigureAwait(false);
        if (routeInfo == null) return null;

        return new GpxTools.FeatureAndBufferedFeature(
            LineFeatureFromFitRoute(routeInfo),
            LineFeatureFromFitRouteBuffered(routeInfo, bufferInFeet));
    }

    public static Task<GpxTools.FeatureAndBufferedFeature?> RouteLineFromFitBuffered(FileInfo fitFile, double bufferInFeet, IProgress<string>? progress = null) =>
        RouteLineFromFitFileBuffered(fitFile, bufferInFeet, progress);

    public static async Task<GpxTools.GpsTrackInformation?> TrackInformationFromFitFile(FileInfo fitFile, IProgress<string>? progress = null)
    {
        if (fitFile is not { Exists: true }) return null;
        ...
        return await Task.Run(() =>
        {
            ...
            {
                startDateTimeUtc = DateTime.SpecifyKind(firstTimestampRec.record.GetTimestamp()!.GetDateTime(), DateTimeKind.Utc);
            }
            ...
            {
                endDateTimeUtc = DateTime.SpecifyKind(lastTimestampRec.record.GetTimestamp()!.GetDateTime(), DateTimeKind.Utc);
            }
            ...
            if (startDateTimeUtc != null)
            {
                try
                {
                    var startTimezoneIanaIdentifier = TimeZoneLookup.GetTimeZone(validRecords[0].lat, validRecords[0].lon);
            ...
            if (endDateTimeUtc != null)
            {
                try
                {
                    var endTimezoneIanaIdentifier = TimeZoneLookup.GetTimeZone(validRecords[^1].lat, validRecords[^1].lon);
            ...
            if (distanceMiles > 0)
            {
                descriptionParts.Add($"Distance: {distanceMiles:0.##} mi");
            }
            ...
            if (climbFeet > 0)
            {
                descriptionParts.Add($"Climb: {Math.Round(climbFeet):N0} ft");
            }
            ...
            if (descentFeet > 0)
            {
                descriptionParts.Add($"Descent: {Math.Round(descentFeet):N0} ft");
            }

            if (session?.GetTotalCalories() is { } cal and > 0)
            {
                descriptionParts.Add($"Calories: {cal}");
            }

            var description = string.Join(", ", descriptionParts);

            return new GpxTools.GpsTrackInformation(name, description, startDateTimeLocal, endDateTimeLocal,
                startDateTimeUtc, endDateTimeUtc, pointList);
        }).ConfigureAwait(false);
    }

    public static Task<GpxTools.GpsTrackInformation?> TrackInformationFromFit(FileInfo fitFile, IProgress<string>? progress = null) =>
        TrackInformationFromFitFile(fitFile, progress);

    public static async Task<Feature?> TrackLineFromFitFile(FileInfo fitFile, IProgress<string>? progress = null)
    {
    ...
    public static Task<Feature?> TrackLineFromFit(FileInfo fitFile, IProgress<string>? progress = null) =>
        TrackLineFromFitFile(fitFile, progress);

    public static async Task<GpxTools.FeatureAndBufferedFeature?> TrackLineFromFitFileBuffered(FileInfo fitFile, double bufferInFeet, IProgress<string>? progress = null)
    {
        var trackInfo = await TrackInformationFromFitFile(fitFile, progress).ConfigureAwait(false);
    ...
    public static Task<GpxTools.FeatureAndBufferedFeature?> TrackLineFromFitBuffered(FileInfo fitFile, double bufferInFeet, IProgress<string>? progress = null) =>
        TrackLineFromFitFileBuffered(fitFile, bufferInFeet, progress);
        ...
        return await Task.Run(() =>
        {
            ...
            foreach (var loopWaypoint in data.CoursePoints)
            {
                ...
                var name = nameBytes != null
                    ? System.Text.Encoding.UTF8.GetString(nameBytes).TrimEnd('\0', ' ')
                    : string.Empty;
        ...
        IProgress<string>? progress = null)
    {
        ...
        return await Task.Run(() =>
        {
            ...
            foreach (var rec in data.Records)
            {
                ...
                {
                    wp = wp.WithTimestampUtc(utcTimestamp);
                }
            ...
            foreach (var cp in data.CoursePoints)
            {
                ...
                {
                    wp = wp.WithDescription(typeString);
                }
    ...
    public static Task<List<GpxWaypoint>> GpxWaypointsFromFit(FileInfo fitFile, IProgress<string>? progress = null) =>
        GpxWaypointsFromFitFile(fitFile, progress);
    ...
    /// </summary>
    public static void WriteActivityFitFile(Stream stream, IEnumerable<GpxTools.GpsTrackInformation> tracks)
    {
    ...
    /// </summary>
    public static void WriteActivityFitFile(FileInfo fitFile, IEnumerable<GpxTools.GpsTrackInformation> tracks)
    {
        ArgumentNullException.ThrowIfNull(fitFile);
        using var stream = fitFile.Open(FileMode.Create, FileAccess.ReadWrite, FileShare.None);
